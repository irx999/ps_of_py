...
class LayerFactory:
    ...
    def _extract_art_layer_info(self, art_layer, parent_path: str = "") -> Dict[str, Any]:
        """提取单个图层的详细信息"""
        layer_info = {
            "name": art_layer.name,
            "type": "ArtLayer",
    ...
    def _extract_layer_set_info(self, layer_set, parent_path: str = "") -> Dict[str, Any]:
        """提取图层集的详细信息及其子图层"""
        current_path = f"{parent_path}/{layer_set.name}" if parent_path != "TOP" else layer_set.name
